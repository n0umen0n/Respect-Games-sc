...
function createGroupsForCurrentWeek(uint256 _communityId) external {
    ...
    console.log("Creating groups for community %s, week %s", _communityId, currentWeek);
...
function updateRespectData(uint256 _communityId, address _user, uint256 _weekNumber, uint256 _respect) external {
    ...
    function getAverageRespect(address _user, uint256 _communityId) public view returns (uint256) {
    ...
    function getCurrentWeek(uint256 _communityId) public view returns (uint256) {
