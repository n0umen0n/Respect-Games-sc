...
(async () => {
  try {
    ...
    const userAccounts = accounts.slice(1, 50); // Use 13 user accounts
    ...
    // Function to determine participating users for each week
    function getParticipatingUsers(userAccounts, weekNumber) {
      if (weekNumber === 1) {
        // In week 1, all even-indexed users participate
        return userAccounts.filter((_, index) => index % 2 === 0);
      } else {
        // In week 2, only the first half of week 1 participants contribute
        const week1Participants = userAccounts.filter((_, index) => index % 2 === 0);
        return week1Participants.slice(0, Math.ceil(week1Participants.length / 2));
      }
    }
    ...
    async function runWeekCycle(weekNumber) {
      ...
      // Get participating users for this week
      const participatingUsers = getParticipatingUsers(userAccounts, weekNumber);
      ...
      for (let i = 0; i < participatingUsers.length; i++) {
        try {
          const userIndex = userAccounts.indexOf(participatingUsers[i]);
          const contribution = [{
            name: `Contribution from User${userIndex + 1} - Week ${weekNumber}`,
            description: `Description of contribution from User${userIndex + 1} for Week ${weekNumber}`,
            links: [`https://example.com/contribution${userIndex + 1}-week${weekNumber}`]
          }];
          await contributionsContract.methods.submitContributions(1, contribution).send({ from: participatingUsers[i], gas: 3000000 });
          console.log(`User${userIndex + 1} submitted a contribution for Week ${weekNumber}`);
          allContributions.push({ user: `User${userIndex + 1}`, contribution: contribution[0] });
        } catch (error) {
          console.error(`Error submitting contribution for User${userIndex + 1} in Week ${weekNumber}:`, error.message);
        }
      }
    ...
    // Log state after change
      ...
      // Update respect data f

      // Verify results and check respect data for all users
      console.log(`\nVerifying results and respect data for Week ${weekNumber}:`);
      const allMembers = await profilesContract.methods.getCommunityMembers(1).call();
      for (const member of allMembers[0]) {
      ...
      if (groups.length > 0) {
        for (let groupId = 0; groupId < groups.length; groupId++) {
          ...
          // Check claimable tokens for group members
